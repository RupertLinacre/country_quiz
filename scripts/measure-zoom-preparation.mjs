...
const reports = []
try {
  for (const variant of ['svg-standard', 'svg-zoom-adaptive', 'svg-zoom-1']) {
    ...
    try {
      ...
      reports.push({ variant, cpuRate: 6, readyMs, preparationMs, jsHeapBytes: heap.usedSize, embedderHeapBytes: heap.embedderHeapUsedSize })
    } finally { await browser.close() }
  }
  await writeFile('output/playwright/zoom-preparation.json', JSON.stringify(reports, null, 2) + '\n')
